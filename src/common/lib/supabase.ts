...
// Create the client, but wrap it in a condition for development mode
export const supabase = createClient(supabaseUrl, supabaseKey);

// Mock Supabase client for development when environment variables aren't set up
export const isMockSupabase = supabaseUrl.includes("placeholder");

if (isMockSupabase) {
  console.warn(
    "Using mock Supabase client. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in your .env file for real functionality."
  );
}
...
export async function signUp(
  ...
  try {
    ...
    if (isMockSupabase) {
      toast.success("Development mode: Simulating successful signup");
      return {
        data: { user: { id: "dev-user-123", email }, session: null },
        error: null,
      };
    }
...
export async function signIn(email: string, password: string) {
  try {
    // Return mock data in development mode
    if (isMockSupabase) {
      toast.success("Development mode: Simulating successful sign in");
      return {
        data: {
          user: { id: "dev-user-123", email },
          session: {
...
// Add a helper function for mocking database operations
function mockDbOperation<T>(
  operation: string,
  mockData: T
): { data: T; error: null } | null {
  if (isMockSupabase) {
    console.log(`Development mode: Simulating ${operation}`);
